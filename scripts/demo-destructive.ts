...
process.env.SAPIENCE_MW_BULK_THRESHOLD = '20';
process.env.OPENCLAW_HOME = fs.mkdtempSync(path.join(os.tmpdir(), 'sapience-mw-demo-'));

async function run(): Promise<void> {
  const { Interceptor } = await import('../src/middlewares/hitl/Interceptor');
  const { createToolCallHook, SAPIENCE_MW_RESPOND_TOOL } = await import('../src/plugin/tool-interceptor');
  ...
  const yes = await hook(
    {
      toolName: SAPIENCE_MW_RESPOND_TOOL,
      params: { decision: 'yes' },
    },
    {
      toolName: SAPIENCE_MW_RESPOND_TOOL,
