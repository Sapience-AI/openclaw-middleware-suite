                ...
                >
                  ...
                  {isOpen && (
                    <div style={{ marginTop: '10px', display: 'grid', gap: '8px' }}>
                      ...
                      <div style={{ borderTop: '1px dashed var(--sai-border)', paddingTop: '8px' }}>
                        <button
                          class="btn-secondary btn-sm"
                          onClick={() =>
                            setAdvancedOpen((a) => ({ ...a, [id]: !a[id] }))
                          }
                        >
                          {advancedOpen[id] ? '▾' : '▸'} Advanced intervention options
                        </button>
                      </div>
                      ...
                      {advancedOpen[id] && (
                        <div style={{ display: 'grid', gap: '8px', paddingLeft: '8px' }}>
                          <label style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px' }}>
                            <input
                              type="checkbox"
                              checked={!!rule.forceAsk}
                              disabled={readOnly}
                              onChange={(e) =>
                                updateRule(mod, method, {
                                  forceAsk: (e.target as HTMLInputElement).checked || undefined,
                                })
                              }
                            />
                            Force ASK even when policy would ALLOW
                          </label>

                          <label style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px' }}>
                            <input
                              type="checkbox"
                              checked={!!rule.requiresExplicitConfirmation}
                              disabled={readOnly}
                              onChange={(e) =>
                                updateRule(mod, method, {
                            ...
                            />
                            Require explicit confirmation token / 2FA
                          </label>

                          <label style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px' }}>
                            <input
                              type="checkbox"
                              checked={!!rule.recommendScreenshotReview}
                              disabled={readOnly}
                              onChange={(e) =>
                                updateRule(mod, method, {
                                  recommendScreenshotReview:
                                    (e.target as HTMLInputElement).checked || undefined,
                                })
                              }
                            />
                            Recommend screenshot review before approval
                          </label>

                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <label style={{ width: '140px', fontSize: '12px' }}>Override description</label>
                            <input
                              class="form-input"
                              style={{ flex: 1 }}
                              type="text"
                              value={rule.overrideDescription || ''}
                              disabled={readOnly}
                              placeholder="Shown to user instead of the default description"
                              onInput={(e) =>
                                updateRule(mod, method, {
                                  overrideDescription:
                          ...
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <label style={{ width: '140px', fontSize: '12px' }}>Intervention reason</label>
                            <input
                              class="form-input"
                              style={{ flex: 1 }}
                              type="text"
                              value={rule.interventionReason || ''}
                              disabled={readOnly}
                              placeholder="Extra reason appended to approval prompt"
                              onInput={(e) =>
                                updateRule(mod, method, {
                                  interventionReason:
                                    (e.target as HTMLInputElement).value || undefined,
                          ...
                          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <label style={{ width: '140px', fontSize: '12px' }}>Action summary</label>
                            <input
                              class="form-input"
                              style={{ flex: 1 }}
                              type="text"
                              ...
                              disabled={readOnly}
                              placeholder="Summary for explicit-confirmation prompts"
                              onInput={(e) =>
                                updateRule(mod, method, {
                                  actionSummary:
                                    (e.target as HTMLInputElement).value || undefined,
