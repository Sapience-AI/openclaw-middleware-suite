...
const generalFields: FormField[] = [
  {
    ...
    description: 'Scan tool call parameters for PII, secrets, and data exfiltration patterns.',
    type: 'toggle',
  },
  {
    key: 'dryRunMode',
...
export function PiiSanitizerPage(_props: { path?: string }) {
  ...
  const saveGeneral = async (val: Record<string, unknown>) => {
    const next: DlpPolicy = { ...policy, enabled: !!val.enabled, dryRunMode: !!val.dryRunMode };
    await save(next);
      ...
      {!loading && tab === 'overview' && (
        <>
          <div class="grid-3 mb-16">
            <StatCard
              label="Status"
              value={policy.enabled ? (policy.dryRunMode ? 'Dry-Run' : 'Enforcing') : 'Disabled'}
            />
            <StatCard
