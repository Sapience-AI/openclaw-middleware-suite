...
// ── Scanner factory ────────────────────────────────────────────────
// `executeGuardrailScan` accepts an optional `configOverride` so the
// per-instance `GuardrailMiddleware` can supply its own merged config
// (defaults < inline < disk + updateConfig() patches). When omitted, falls
// back to the static `ConfigStore.getCached()` — backward-compatible for
// direct programmatic consumers of `executeGuardrailScan`.
...
function preReadAndScan(
  ...
  for (const filePath of filePaths) {
    ...
    // Map guardrail severity (LOW|MEDIUM|HIGH|CRITICAL) to DecisionLog
    // severity (LOW|HIGH|CATASTROPHIC). CRITICAL → CATASTROPHIC, others → HIGH.
    const topSeverity = detections.some((d) => d.severity === 'CRITICAL')
      ? ('CATASTROPHIC' as const)
      : ('HIGH' as const);

    if (hasBlock) {
      const blockReason = `Sapience Guardrail blocked ${toolName}('${basename}'): file content contains [${reason}]`;
      void DecisionLog.append({
        timestamp: new Date().toISOString(),
        module: moduleName,
        method: methodName,
        args: [{ toolName, filePath: resolvedPath! }],
        decision: 'BLOCKED',
        decisionTime: 0,
        reason: `guard:pre-read: ${blockReason}`,
        eventType: 'tool_blocked',
        tool: toolName,
        severity: topSeverity,
        reasons: detections.map((d) => `${d.category}:${d.ruleName}`),
        agentId,
        sessionKey,
      });
      return {
    ...
    if (hasWarn) {
      const warnReason = `Sapience Guardrail warning on ${toolName}('${basename}'): file content contains [${reason}]`;
      void DecisionLog.append({
        timestamp: new Date().toISOString(),
        module: moduleName,
        method: methodName,
        ...
        decision: 'ALLOWED',
        decisionTime: 0,
        reason: `guard:pre-read (escalate): ${warnReason}`,
...
export function executeGuardrailScan(
  ...
  try {
    const loaded = getScanner(configOverride);
    ...
    const preReadResult = preReadAndScan(
      toolName,
      moduleName,
      methodName,
      params,
