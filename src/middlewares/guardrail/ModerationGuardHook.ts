...
import { AgentStartContext, LifecycleContext } from '../../types.js';
 ...
 * before any before_message_write handler runs for this turn. Signature
 * matches OpenClaw's raw `(event, ctx)` dispatch; both args are typed
 * with shared `LifecycleContext`-derived shapes from `src/types.ts`.
 */
