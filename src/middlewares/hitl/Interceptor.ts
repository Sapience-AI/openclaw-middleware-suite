...
export class Interceptor {
  ...
  /**
   * Live plugin-level enabled flag read from `plugin_config.middlewares.hitl`.
   * Refreshed by the plugin-level `ConfigStore.onChange('hitl', …)` callback so
   * dashboard toggles take effect without a process restart. The composed
   * tool-call hook gates on this because OpenClaw doesn't deregister hooks
   * when a middleware is toggled off — the hook itself has to bail.
   */
  ...
  /** Live plugin-level enable flag. Set by the plugin's onChange callback. */
