...
const DEFAULT_LEVEL_1 = 3;
const DEFAULT_LEVEL_2 = 5;
...
export class TrustRateLimiter {
  ...
  recordDenial(
    sessionKey: string,
    level1Threshold = DEFAULT_LEVEL_1,
    level2Threshold = DEFAULT_LEVEL_2
  ): void {
    const now = Date.now();
    ...
    const level = this.computeLevel(state, level1Threshold, level2Threshold);
  ...
  getLevel(
    ...
    return this.computeLevel(state, level1Threshold, level2Threshold);
  }
  ...
  getState(
    ...
    return {
      level: this.computeLevel(state, level1Threshold, level2Threshold),
      denialCount: state.timestamps.length,
  ...
  private computeLevel(
    state: SessionDenialState,
    level1Threshold: number,
    level2Threshold: number
  ): EscalationLevel {
    const count = state.timestamps.length;
    if (count >= level2Threshold) return 2;
    if (count >= level1Threshold) return 1;
    return 0;
  }
}
