...
/**
 * Sapience Middleware Policy Management Command
 */
...
export async function policyCommand(): Promise<void> {
  ...
  console.log(chalk.bold.cyan('   🔒 HITL Policy Manager'));
  console.log(chalk.bold.cyan('═'.repeat(80)));
  console.log('');
...
async function changeDefaultLevel(policy: PersistedPolicy): Promise<void> {
  ...
  console.log(
    chalk.yellow('⚠️  Restart OpenClaw for changes to take effect: openclaw gateway restart')
  );
}
