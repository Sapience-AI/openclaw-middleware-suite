...
/**
 * Sapience Middleware Destructive Action Classifier
...
function collectNumericSignals(
  ...
  if (typeof value === 'string') {
    ...
    while ((destructiveMatch = destructiveCountRegex.exec(value)) !== null) {
      signals.push({
        path: `${currentPath || 'string'}.destructive_count`,
        value: Number(destructiveMatch[1]),
      });
    }

    const itemCountRegex =
      /\b(\d{2,})\s*(emails?|messages?|items?|records?|rows?|users?|files?)\b/gi;
    let itemMatch: RegExpExecArray | null;
    while ((itemMatch = itemCountRegex.exec(value)) !== null) {
      signals.push({
        path: `${currentPath || 'string'}.${itemMatch[2]}`,
...
function extractTarget(toolName: string, args: unknown): string | undefined {
  ...
  const pathMatch = /((?:\/|~\/)[\w./-]+|[a-zA-Z]:\\[^\s]+)/.exec(fullText);
  if (pathMatch) {
    return pathMatch[1];
  }
...
export function classifyDestructiveAction(
  ...
  const bulkThreshold = meta.bulkThreshold ?? DEFAULT_BULK_THRESHOLD;
  ...
  const isDeleteContext =
    containsRiskVerb(text) || /delete|remove|rm|purge|trash|empty|wipe/i.test(methodName);
  const bulkCount = detectBulkCount(args, bulkThreshold);
  if (bulkCount !== undefined && isDeleteContext) {
  ...
  if (normalizedTool.includes('gmail') || moduleName === 'gmail') {
    ...
    if (
      (normalizedTool.includes('deletemessages') ||
  ...
  // Google Drive catastrophic buckets
  if (
    normalizedTool.includes('gdrive') ||
    normalizedTool.includes('drive') ||
    moduleName === 'googledrive'
  ) {
    if ((methodName === 'delete' || /\bdelete\b/.test(text)) && bulkCount !== undefined) {
      catastrophic = true;
      pushUnique(reasons, 'gdrive_bulk_delete');
    }
    if (/empty\s*trash|purge|delete\s*all|trash\s*all/.test(text)) {
      catastrophic = true;
      pushUnique(reasons, 'gdrive_full_purge');
    }
    if (
      /\banyone\b|\bpublic\b/.test(text) &&
      (methodName === 'share' || /\bshare\b|\bpermission/.test(text))
    ) {
      catastrophic = true;
      pushUnique(reasons, 'gdrive_public_share');
    }
  }
