...
import { IrreversibilityAssessment } from './IrreversibilityScorer.js';
...
function tokenize(text: string): Set<string> {
  const tokens = (text.toLowerCase().match(/[a-z0-9_]+/g) || []).filter(
...
export class MemoryRiskForecaster {
  ...
  assess(
    ...
    const shouldPause =
      overallRisk >= attackPauseThreshold || (topSimulatedRisk >= 88 && irreversibility.score > 20);
  ...
  private computeSalamiIndex(state: SessionMemoryState): number {
    ...
    const mostlyLowRiskSteps =
      window.filter((event) => event.irreversibility < 55).length >= Math.ceil(window.length * 0.7);
  ...
  private computeCommitmentCreep(state: SessionMemoryState): number {
    ...
    const avg =
      irreversibilityValues.reduce((acc, value) => acc + value, 0) / irreversibilityValues.length;
    const slope =
      irreversibilityValues.length >= 2
  ...
  private simulateNextTurnPaths(state: SessionMemoryState): SimulatedPath[] {
    ...
    if (
      containsAny(text, [
        /wire transfer|bank transfer|payment|invoice|routing number|swift|\bach\b/i,
      ])
    ) {
      candidates.push({
    ...
    if (
      stageSet.has('destruct') ||
      containsAny(text, [/rm -rf|delete|drop table|truncate|wipe|\bformat\b(?!\s*[-=])/i])
    ) {
      candidates.push({
  ...
  private detectStages(moduleName: string, methodName: string, text: string): AttackStage[] {
    ...
    if (
      ...
      (lowerModule === 'gateway' &&
        (lowerMethod === 'listsessions' || lowerMethod === 'listnodes')) ||
    ...
    if (
      (lowerModule === 'network' &&
        (lowerMethod === 'request' || lowerMethod === 'webhook' || lowerMethod === 'download')) ||
    ...
    if (
      ...
      (lowerModule === 'shell' &&
        ...
        containsAny(text, [/rm -rf|del \/s|drop table|truncate|wipe|\bformat\b(?!\s*[-=])|shred/i]))
    ) {
      stages.add('destruct');
    }
