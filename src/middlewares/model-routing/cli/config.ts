...
export async function routerConfigCommand(opts: {
  ...
  if (opts.enablePinning || opts.disablePinning) {
    ...
    store.setSessionPinningEnabled(enabled);
    ...
    if (!enabled) store.setProviderCacheEnabled(false);
    await store.save();
    console.log(
      chalk.green(
  ...
  if (opts.enableCache || opts.disableCache) {
    ...
    store.setProviderCacheEnabled(enabled);
    await store.save();
  ...
  if (opts.setWeight) {
    ...
    store.setWeightOverride(dimName, weight);
    await store.save();
  ...
  if (opts.setProfile) {
    ...
    store.setDefaultProfile(opts.setProfile as RoutingProfile);
    await store.save();
  ...
  if (opts.setBoundary) {
    ...
    store.setBoundaryOverride(name, value);
    await store.save();
