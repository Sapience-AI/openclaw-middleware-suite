...
export const dlpInfoCommand = async () => {
  ...
  console.log(
    chalk.bold('State:'),
    policy.enabled ? chalk.green('ENABLED') : chalk.red('DISABLED')
  );
  console.log(
    chalk.bold('Mode: '),
    policy.dryRunMode
...
export const dlpToggleCommand = async (setting: string) => {
  ...
  if (setting === 'enable') {
    policy.enabled = true;
    console.log(
      chalk.bgGreen.black.bold(' 🛡️ DLP ENABLED ') +
        chalk.green(' Personal data protection is now ACTIVE.')
    );
  } else if (setting === 'disable') {
    policy.enabled = false;
    console.log(
      chalk.bgRed.black.bold(' 🛡️ DLP DISABLED ') +
        chalk.red(' Security middleware is now BYPASSED.')
    );
    ...
    console.log(
      chalk.red.bold('! ERROR: ') + `Unknown setting "${setting}". Use enable, disable, or dry-run.`
    );
    return;
  }
