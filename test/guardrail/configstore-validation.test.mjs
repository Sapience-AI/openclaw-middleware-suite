...
test('ConfigStore: invalid root config falls back to defaults', async () => {
  await withConfig('not-an-object', async () => {
    ...
    // The per-middleware `enabled` flag was removed; the plugin-level toggle
    // now lives at plugin_config.middlewares.guardrail. Validate via fields
    // that *are* on GuardrailConfig.
    assert.equal(c.entropyThreshold, 4.0);
    assert.equal(c.dryRunMode, false);
  });
});

test('ConfigStore: invalid dryRunMode type warned, default kept', async () => {
  await withConfig({ dryRunMode: 'maybe' }, async () => {
    const c = await cs.ConfigStore.load();
    assert.equal(c.dryRunMode, false);
  });
});
