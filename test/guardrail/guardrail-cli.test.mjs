...
test('guardrail toggle enable/disable/dry-run', async () => {
  await capture(() => toggle.guardrailToggleCommand('enable'));
  assert.equal((await ConfigStore.load()).enabled, true);
  await capture(() => toggle.guardrailToggleCommand('disable'));
  assert.equal((await ConfigStore.load()).enabled, false);
