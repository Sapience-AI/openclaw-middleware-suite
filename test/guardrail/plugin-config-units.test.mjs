...
const { executePiiScan, piiSanitizer } = await import(u('middlewares/pii-sanitizer/PiiSanitizerHook.js'));
...
test('PiiSanitizerHook: executePiiScan with clean params', async () => {
  // Wait for sanitizer init
  await new Promise(r => setTimeout(r, 100));
  const r = await executePiiScan('read', 'FileSystem', 'read', { path: '/tmp/x' });
  assert.ok(r);
  assert.ok(typeof r.modified === 'boolean');
});

test('PiiSanitizerHook: executePiiScan with credit card triggers redaction', async () => {
  const r = await executePiiScan('write', 'FileSystem', 'write', {
    content: 'My card is 4532-1234-5678-9010',
  });
  assert.ok(r);
  // params should be returned (redacted or not)
  assert.ok(r.params);
});

test('PiiSanitizerHook: executePiiScan with private key triggers BLOCK', async () => {
  const r = await executePiiScan('write', 'FileSystem', 'write', {
    content: '-----BEGIN RSA PRIVATE KEY-----\nMIIEpAIBAAKCAQEA',
  });
  assert.ok(r);
  // either blocks or modifies
  assert.ok(r.params || r.block);
});
