...
import { runCli as _runCli } from '../_helpers/cli-runner.mjs';
...
function runCli(args, env) {
  return _runCli(cliPath, repoRoot, args, env);
}
...
test('sapience-middleware configure --non-interactive --json succeeds in temp OPENCLAW_HOME', () => {
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'sapience-mw-configure-success-'));
  ...
  const result = runCli(['configure', '--non-interactive', '--json'], {
    ...
    OPENCLAW_PLUGIN_ID: 'sapience-middleware',
    OPENCLAW_PLUGIN_DIR: path.join(openclawHome, 'extensions', 'sapience-middleware'),
    LOG_LEVEL: 'error',
  });
...
test('sapience-middleware configure --non-interactive --json fails for custom security without modules', () => {
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'sapience-mw-configure-fail-'));
  const openclawHome = path.join(tempRoot, 'openclaw-home');
  const openclawConfig = path.join(openclawHome, 'openclaw.json');
