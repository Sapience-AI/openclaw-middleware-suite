...
test('HitlMiddleware', async (t) => {
  ...
  await t.test('initialize ignores config.enabled (HITL is a dumb-delegator class)', async () => {
    // Post-Pass-1: HitlMiddleware no longer has an internal `enabled` gate.
    // The real plugin-runtime gate is Interceptor.isPluginEnabled() upstream
    // in the composed tool-call hook; programmatic users disable by simply
    // not calling beforeToolCall(). getStatus() always reports `enabled: true`.
    const mw = new HitlMiddleware();
    await mw.initialize({ enabled: false });
    assert.equal(mw.getStatus().enabled, true);
  });
