...
import { createOpenclawHome, disableDestructiveGating } from '../_helpers/test-env.mjs';

const openclawHome = createOpenclawHome('sapience-mw-policy-tests-');
// Disable destructive gating so classifier doesn't override policy decisions
disableDestructiveGating();
...
const allowedDir = mkdtempSync(path.join(os.tmpdir(), 'sapience-mw-allowed-'));
const outsideDir = mkdtempSync(path.join(os.tmpdir(), 'sapience-mw-outside-'));
