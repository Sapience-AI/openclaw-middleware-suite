...
const { executePiiScan } = await import(
  new URL(`file:///${distBase}/middlewares/pii-sanitizer/PiiSanitizerHook.js`).href
);
const { executeLimitCheck } = await import(
  new URL(`file:///${distBase}/middlewares/tool-call-limit/ToolCallLimitHook.js`).href
);
...
function buildComposedPipeline(opts = {}) {
  ...
  return async (event, ctx) => {
    ...
    if (piiSanitizer) {
      try {
        const piiResult = await executePiiScan(
          event.toolName || event.tool || '',
          event.moduleName || '',
          event.methodName || '',
          event.params || event.input || {},
    ...
    if (toolCallLimit) {
      try {
        const limitResult = await executeLimitCheck(
          event.toolName || event.tool || '',
          event.moduleName || '',
          event.methodName || '',
          event.params || event.input || {},
...
describe('Group F: State Preservation', () => {
  ...
  test('F2: PII singleton survives multiple invocations without double-init errors', async () => {
    // Call executePiiScan twice in sequence
    let threw = false;
    try {
      await executePiiScan('read', 'FileSystem', 'read', { path: '/tmp/a.txt' }, 'sess1');
      await executePiiScan('read', 'FileSystem', 'read', { path: '/tmp/b.txt' }, 'sess2');
    } catch {
      threw = true;
    }
