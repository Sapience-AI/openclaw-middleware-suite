...
test('Reproduction - PII Escalation Loop via Magnitude Regex', async (t) => {
  ...
  await t.test('System messages containing magnitude words should NOT escalate', async () => {
    ...
    // REDACT is okay, but ESCALATE is what causes the loop (since it blocks for human approval).
    // In CURRENT implementation, magnitude word "entire" triggers ESCALATE.
    if (result.block && result.metadata?.piiAction === 'ESCALATE') {
